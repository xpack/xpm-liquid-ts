...
The `xpm.Package` is the top object that includes all xpm specific definitions
in the `package.json` file.

To import its definition in a file, use:

```typescript
import * as xpm from '@xpack/xpm-lib'
```
...
```typescript
const xpmPackage = new xpm.Package({ log, packageFolderPath: config.cwd })
```
...
```typescript
const xpmDataModel = new xpm.DataModel({ log, jsonPackage })
```
