...
* identify the desired version
  of @xpack/xpm-lib, download it into a cache and unpack
...
* create a local symbolic link
  such as <code>xpacks/@xpack/xpm-lib</code> pointing to the
  versioned folder in the user's global xPacks store
* add <code>@xpack/xpm-lib</code> to
  `package.json` as a
  dependency; this associates a specific version of xpm Core Library with
